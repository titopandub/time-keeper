...
formatTime time =
    let
        ...
        minutes =
            floor ((roundedTime - toFloat (hours)) * 60)
    in
...
calcAsrTime latitude factor julianDate time =
    let
        sunPositionAtTime =
            sunPosition (julianDate + time)

        declination =
            sunPositionAtTime.declination

        absLatDec =
            (abs (latitude - declination))

        degreeTan =
            (tan ((absLatDec * pi) / 180))

        angle =
            (-1 * ((atan (1 / (factor + degreeTan)) * 180) / pi))
    in
...
sunAngleTime latitude angle julianDate time direction =
    let
        ...
        beforeArcCos =
            ((degreeA - degreeSinD * degreeSinL) / (degreeCosD * degreeCosL))

        tVar =
            (1 / 15 * ((acos beforeArcCos) * 180 / pi))
    in
        case direction of
            CCW ->
                (noon + (-1 * tVar))
...
type alias SunPosition =
    { declination : Float, equation : Float }
    ...
    let
        ...
        eclipticLongitude =
            let
                ...
                y =
                    0.02 * (sin (degrees (2 * meanAnomaly)))
            in
